  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 pb-4 border-b border-gray-200">
        ...
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Financial & Tokenization</h2>
          <p className="text-sm text-gray-600">البيانات المالية والتوكنة - Valuation and token structure</p>
        </div>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-start gap-3">
        ...
        <div className="text-sm text-blue-800">
          <p className="font-medium mb-1">Tokenization Structure</p>
          <p>Define how your property will be tokenized. The system will calculate available tokens based on your retained percentage.</p>
        </div>
      </div>
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">
          Market Value (SAR) / القيمة السوقية <span className="text-red-500">*</span>
        </label>
        <input
          type="number"
      ...
      <div className="space-y-4 pt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Total Tokens / إجمالي الرموز <span className="text-red-500">*</span>
            </label>
            <input
              type="number"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Token Price (SAR) / سعر الرمز <span className="text-red-500">*</span>
            </label>
            <input
              type="number"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            Owner Retained Percentage (%) / نسبة احتفاظ المالك <span className="text-red-500">*</span>
          </label>
          <input
            type="number"
          ...
          <p className="mt-1 text-xs text-gray-500">
            Percentage of tokens you want to retain for yourself (0-100%)
          </p>
          ...
          <div className="bg-gradient-to-br from-emerald-50 to-blue-50 border border-emerald-200 rounded-lg p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white rounded-lg p-4">
                <p className="text-sm text-gray-600 mb-1">Available for Sale</p>
                <p className="text-2xl font-bold text-emerald-600">
                  {calculations.availableTokens.toLocaleString()} tokens
                </p>
                <p className="text-xs text-gray-500 mt-1">
                  Value: {calculations.totalTokenValue.toLocaleString()} SAR
                </p>
              </div>
              <div className="bg-white rounded-lg p-4">
                <p className="text-sm text-gray-600 mb-1">Owner Retained</p>
                <p className="text-2xl font-bold text-blue-600">
                  {calculations.retainedTokens.toLocaleString()} tokens
                </p>
                <p className="text-xs text-gray-500 mt-1">
                  Value: {calculations.ownerRetainedValue.toLocaleString()} SAR
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="space-y-4 pt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Expected Annual ROI (%) / العائد السنوي المتوقع <span className="text-red-500">*</span>
            </label>
            <input
              type="number"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Expected Monthly Yield (%) / العائد الشهري المتوقع <span className="text-red-500">*</span>
            </label>
            <input
              type="number"
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            <div className="flex items-start gap-2">
              ...
              <div className="text-sm text-yellow-800">
                <p className="font-medium mb-1">ROI Consistency Check</p>
                <p>
                  Monthly yield × 12 = {(Number(formData.expectedMonthlyYield) * 12).toFixed(2)}%
                  {Math.abs((Number(formData.expectedMonthlyYield) * 12) - Number(formData.expectedROI)) > 0.5 && (
                    <span className="text-red-600 font-medium"> ⚠️ This doesn't match your annual ROI ({formData.expectedROI}%)</span>
                  )}
                </p>
              </div>
            </div>
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">
          Payout Schedule / جدول التوزيع <span className="text-red-500">*</span>
        </label>
        <select
